...
function factory(Promise, utils)
{
  ...
  function getFields(record)
  {
    return record.fields.filter(function(field) {
      return field.tag === '700' && field.subfields.some(function(subfield) {
        return subfield.code === 'e' && subfield.value.indexOf(',,') >= 0;
      });
    });
  }
      ...
      return {
        validate: function(record)
        {
          return Promise.resolve(getFields(record).map(function(field) {
            return utils.validate.warning("Double comma at subfield 'e'", field);
          }));
        },
        fix: function(record)
        {
          return Promise.resolve(getFields(record).map(function(field) {
            return utils.fix.modifySubfields(field, function(subfield) {
              if (subfield.code === 'e' && subfield.value.indexOf(',,') >= 0) {
