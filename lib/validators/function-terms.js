...
function factory(Promise, shim_array, utils, _)
{
  ...
  return {
    ...
    factory: function() {
      return {
        ...
        fix: function(record) {
          var results = getFields(record).map(function(field) {
            var res = utils.fix.modifySubfields(field, function(subfield) {
              if (subfield.code === TAG_SUBFIELD_MAP[field.tag]) {
                var normalized = normalizeValue(subfield.value);
                if (Object.keys(FUNCTION_TERM_MAP).indexOf(normalized) > -1) {
                  subfield.value = subfield.value.replace(new RegExp(subfield.value, 'g'), FUNCTION_TERM_MAP[normalized]);
                }
              }
            });
            // De-duplicate identical subfields.
            field.subfields = _.uniqBy(field.subfields, function(sf) {
              return sf.value;
            });
            return res;
          });
