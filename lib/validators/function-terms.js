          ...
          getFields(record).forEach(function(field) {            
            field.subfields.forEach(function(subfield) {
              ...
              if (subfield.code === TAG_SUBFIELD_MAP[field.tag]) {
                ...
                value_normalized = normalizeValue(subfield.value);              
                value_normalized = value_normalized.replace(new RegExp(value_normalized, 'g'), FUNCTION_TERM_MAP[value_normalized]);
                
                results.push(utils.fix.modifySubfieldValue(field, subfield, value_normalized));
