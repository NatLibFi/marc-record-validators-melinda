      ...
      return {
        ...
        fix: function(record) {
          return Promise.resolve(getFields(record).map(function(field) {
            var newSubfield = {
              code: '2',
              value: RECORD_TYPES.includes(record.leader.slice(6,8)) ? '1974/fin/finuc-s' : '1974/fin/fennica'
            };
            var keepIndex = field.subfields.findIndex(function(sf) { return sf.value === 'FENNI<KEEP>'; });
            field.subfields.splice(keepIndex, 0, newSubfield); // Append the new subfield $2 in front of the $9 FENNI<KEEP> subfield.
            return {
              type: 'addSubfield',
