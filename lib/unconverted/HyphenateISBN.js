...
// isbn-groups.js from https://code.google.com/p/isbnjs/
  ...
  "99962": {
    ...
    var targets = [
      ...
      { field: '787', subfield:'z' },
		...
		var debug = 1;
		var isbnish = /^[0-9]{9}([0-9][0-9][0-9])?[0-9X]$/;
		function requiresAction(recordModel, field, fieldCode, subfieldCode) {
			...
			if ( isbnish.exec(sf.content) ) {
				var isbn = ISBN.parse(sf.content);
        if ( isbn ){
          if ( isbn.isIsbn10() ) {
            //console.info("test "+sf.content+" => "+ISBN.hyphenate(sf.content));
            return 10;
          } 
          if ( isbn.isIsbn13() ) {
            //console.info("test "+sf.content+" => "+ISBN.hyphenate(sf.content));
          ...
          console.info("NB! "+sf.content+" was not accepted as ISBN!");
        } 
			}	
      ...
      for (i=0; i < targets.length; i++) {
        ...
        if ( result ) {
          var msg = "020: tavuta ISBN '" + RecordModel.fieldToString(field)+"'";		
      ...
      for (i=0; i < targets.length; i++) {
				...
				if ( result ) {
					...
					var hyphenatedISBN = ISBN.hyphenate(sf.content);
					if ( hyphenatedISBN ) {
						if ( debug ) {
							console.info("HyphenateISBN fixer, "+targets[i].field+targets[i].subfield+": "+sf.content+" => "+hyphenatedISBN);
						}
						sf.content = hyphenatedISBN;
						hits++;
					}
					else {
            console.info("Error: no hyphenation for "+sf.content)
					}
				}
			}
