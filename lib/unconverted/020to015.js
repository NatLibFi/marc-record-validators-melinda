/*
 *
 * 020to015.js -- shift ISSN from 020 to it's rightful place as 022
 * 
	...
	"core/L",
		...
		function requiresAction(recordModel, field) {
			...
			if ( recordModel.getSubfieldByCode(field, 'z') ) {
				return 0; // Don't dare. These seem to be ISBNs.
			} 

			if ( issnish.exec(sf.content) ) {
				console.info("020a on 015! "+sf.content);
				return 1;
			} 
			return 0;
		}
		...
		function fix (action, recordModel, field, i) {
			...
			if ( result ) {
        field.tag = '015';
        field.subfields.push(recordModel.createSubfield("2", 'skl')); // "sid." tms.
        if ( debug ) {
          console.info('020to015 fixer: '+RecordModel.fieldToString(field));
        }
