...
export default async function (isLegalDeposit = false) {
    ...
    if (f856sUrn.length === 0) {
      ...
      f856sUrn.forEach(f => {
        ldSubfields.forEach(ldsf  => {
          if (!f.subfields.some(sf => { return (sf.code === ldsf.code) && (sf.value === ldsf.value); })) {
  ...
  const hasURN = f => { return (f.tag === '856') && f.subfields.some(({code, value}) => { return (code === 'u') && /urn.fi/.test(value); }); };
