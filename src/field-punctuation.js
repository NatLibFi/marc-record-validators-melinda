...
export default function () {
	function fix(record) {
		const rules = readPunctuationRulesFromJSON(require('../src/bib-punctuation.json'));
		record.fields.map(field => punctuateField(field, rules));
		return true;

		function readPunctuationRulesFromJSON(json) {
		...
		function punctuateField(field, rules, recordType = 'BIBLIOGRAPHIC') {
	...
	return {
		...
		validate: async record => ({
			valid: record.fields.some(f => (f.tag === '100' && f.subfields.some(sf => sf.code === 'a' && sf.value.endsWith(','))) || (f.tag === '264' && f.subfields.some(sf => sf.code === 'b' && sf.value.endsWith(','))))
		}),
		fix
	};
}
